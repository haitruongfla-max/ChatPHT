...
function VideoBubble({ uri, onOpen }: { uri: string; onOpen: () => void }) {
  return <Pressable onPress={onOpen} style={styles.videoFrame} accessibilityRole="button" accessibilityLabel="Mở video toàn màn hình"><View pointerEvents="none" style={styles.videoPreview}><View style={styles.videoIcon}><MaterialIcons name="play-arrow" size={30} color="#FFFFFF" /></View><Text style={styles.videoPreviewText}>Chạm để xem video</Text></View></Pressable>;
}
...
function MessageTime({ item, mine }: { item: ChatMessage; mine: boolean }) {
  ...
  const deliveryState = item.recipientReadAt && new Date(item.recipientReadAt).getTime() >= createdAt
    ? "Đã đọc"
    : item.recipientDeliveredAt && new Date(item.recipientDeliveredAt).getTime() >= createdAt
      ? "Đã nhận"
      : "Đã gửi";

  return <View style={styles.messageMeta}><Text style={[styles.messageTime, mine && styles.mineTime]}>{relativeTime(item.createdAt)}</Text>{mine ? <Text style={styles.deliveryState}>{deliveryState}</Text> : null}</View>;
}
...
export default function ChatScreen() {
  ...
  useEffect(() => {
    if (!userId || messageCount === 0 || !Number.isInteger(conversationId)) return;
