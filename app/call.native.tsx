...
import { ActivityIndicator, Alert, Animated, Image, Platform, Pressable, SafeAreaView, StatusBar, StyleSheet, Text, View, type StyleProp, type TextStyle, type ViewStyle } from "react-native";
...
function P2pVideoStage({ localStream, remoteStream, localScreenStream, remoteScreenStream }: { localStream: MediaStream | null; remoteStream: MediaStream | null; localScreenStream: MediaStream | null; remoteScreenStream: MediaStream | null }) {
  const localScreenSharing = Boolean(localScreenStream);
  // Do not render the sender's display stream back to itself: Android would
  // capture this RTCView again and create an infinite mirror effect.
  const main = remoteScreenStream ?? (localScreenSharing ? null : remoteStream);
  const corner = localStream;
  const shareLabel = remoteScreenStream
    ? "Đang xem màn hình được chia sẻ"
    : localScreenSharing
      ? "Đang phát màn hình cho người nhận"
      : null;
  return <View style={styles.video}>{main ? <RTCView streamURL={main.toURL()} style={styles.videoTrack} objectFit={remoteScreenStream ? "contain" : "cover"} /> : localScreenSharing ? <View style={styles.localShareShield}><MaterialIcons name="screen-share" size={42} color="#BFDBFE" /><Text style={styles.localShareTitle}>Bạn đang chia sẻ màn hình</Text><Text style={styles.localShareCopy}>Người nhận đang xem màn hình của bạn. Bản xem trước được ẩn để tránh hiệu ứng lặp.</Text></View> : <View style={styles.waiting}><MaterialIcons name="videocam-off" size={32} color="#D9E6FF" /><Text style={styles.waitingText}>Đang thiết lập hình ảnh P2P…</Text></View>}{corner ? <View style={styles.localPreview}><RTCView streamURL={corner.toURL()} style={styles.videoTrack} objectFit="cover" /></View> : null}{shareLabel ? <Text style={styles.shareLabel}>{shareLabel}</Text> : null}</View>;
}
