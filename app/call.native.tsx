...
const MODE_IDENTITY: Record<P2pCallMode, { label: string; icon: React.ComponentProps<typeof MaterialIcons>["name"] }> = {
  audio: { label: "GỌI THOẠI · CHỈ MICRO", icon: "phone" },
  video: { label: "GỌI VIDEO · CAMERA + MICRO", icon: "videocam" },
  screen: { label: "CHIA SẺ MÀN HÌNH · MODE RIÊNG", icon: "screen-share" },
};
...
export default function CallScreen() {
  ...
  async function startP2p(isAnswer: boolean) {
    ...
    await p2p.start({
      ...
      onState: (state) => {
        ...
        if (state !== "connected") setLatencyMs(null);
      ...
      onStats: (stats: P2pNetworkStats) => setLatencyMs(stats.latencyMs),
    });
  ...
  async function enterCall(isAnswer: boolean) {
    ...
    if (modeConflict) {
      Alert.alert("Đã chặn mở sai chế độ", "Phiên gọi không khớp với nút bạn vừa bấm. Hãy đóng phiên này và bắt đầu lại từ đúng nút thoại, video hoặc chia sẻ màn hình.");
      return;
    }
  ...
  async function toggleScreenCamera() { const next = !cameraOn; try { if (next) { const camera = await Camera.requestCameraPermissionsAsync(); if (!camera.granted) { Alert.alert("Cần quyền camera", "Hãy cấp quyền camera để bật cửa sổ camera phụ khi chia sẻ màn hình."); return; } } await p2p.setScreenCameraEnabled(next); setCameraOn(next); activeCall.update(callId, { cameraOn: next }); } catch (error) { Alert.alert("Chưa thể đổi camera phụ", error instanceof Error ? error.message : "Hãy kiểm tra quyền camera rồi thử lại."); } }
...
function ModeIdentityBadge({ mode, inverse = false }: { mode: P2pCallMode; inverse?: boolean }) {
  const identity = MODE_IDENTITY[mode];
  return <View accessibilityLabel={`Chế độ đã khóa: ${identity.label}`} style={[styles.modeIdentityBadge, inverse && styles.modeIdentityBadgeInverse]}><MaterialIcons name={identity.icon} size={15} color={inverse ? "#D8E7FF" : "#1D4ED8"} /><Text style={[styles.modeIdentityText, inverse && styles.modeIdentityTextInverse]}>{identity.label}</Text></View>;
}
